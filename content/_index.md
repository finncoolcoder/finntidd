+++
title = "FinnTidd.world"
+++

# Welcome to finntidd.world

This website is a collection of everything I do, documented so its easier for you to do it too.

## stuff

* [My posts](/posts/)
* [GitHub](https://github.com/finncoolcoder)
* [Instagram](https://instagram.com/finntidd1)
* [YouTube](https://youtube.com/@finndolf/)



<a title="free world map tracker" href="https://smallcounter.com/vmap/1789314130/"> <img title="free world map counter" src="https://smallcounter.com/map/view.php?type=180&id=1789314130" border="1" alt="world map hits counter" /></a><br> <a href="https://smallcounter.com/map/">map counter</a> 
  <iframe src="https://www3.cbox.ws/box/?boxid=3540448&boxtag=fCZrm2" width="100%" height="450" allowtransparency="yes" allow="autoplay" frameborder="0" marginheight="0" marginwidth="0" scrolling="auto"></iframe>	
<a href="https://open.spotify.com/user/31d3iq6hitkh6jqgz4l5wl5rzoui">
  <img src="https://spotify-recently-played.jeffreyca.workers.dev/svg?user=31d3iq6hitkh6jqgz4l5wl5rzoui" alt="Spotify recently played" width="400" />
</a>
<div style="width: fit-content; border: 2px outset; text-align:center">
	<p style="margin: 0; padding: 0.1em; border: 2px inset">This site is a member of Crispy Onion Ring.</p>
	<div style="margin: 0; padding: 0.1em; border: 2px inset">
		<a href="https://webri.ng/webring/onionring/random">Random Site</a>
	</div>
    <div style="font-size: 22px;">
    <a href="https://cryptidthrifter.neocities.org/webring/diyring" target="_blank"><img src="https://cryptidthrifter.neocities.org/webring/diyring.png"></a><br />
    <a href="https://cryptidthrifter.neocities.org/webring/diyring?action=previous" target="_blank">⇐</a>
    <a href="https://cryptidthrifter.neocities.org/webring/diyring?action=random" target="_blank">random</a>
    <a href="https://cryptidthrifter.neocities.org/webring/diyring?action=next" target="_blank">⇒</a>
</div>
        <map name="badge-geekring">
        <area shape="rect" coords="4,3,13,28" href="http://geekring.net/site/NUMBER/previous" alt="Previous">
        <area shape="rect" coords="13,3,22,28" href="http://geekring.net/site/NUMBER/random" alt="Random">
        <area shape="rect" coords="76,3,84,28" href="http://geekring.net/site/NUMBER/next" alt="Next">
        <area shape="rect" coords="23,3,63,28" href="https://geekring.net/" alt="Geekring">
   </map>
  <img usemap="#badge-geekring" src="http://geekring.net/banner/geek_2.gif" alt="The Geekring">

<a href="https://bungle.online/clique"><img src="https://bungle.online/clique/img/outline_3.png" alt="Angel resting upon a computer"></a>
more stuff coming soon...
